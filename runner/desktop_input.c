...
static bool GamepadBindingPressed(const Dkc1GamepadState *gamepad,
                                  int binding, int16_t deadzone,
                                  uint8_t trigger_threshold) {
  if (!gamepad || binding <= 0) return false;
  if (DKC1_PAD_IS_BUTTON(binding)) {
    uint32_t mask = 0;
    switch (DKC1_PAD_BUTTON_CODE(binding)) {
      case kStandardPadA: mask = kDkc1GamepadA; break;
      case kStandardPadB: mask = kDkc1GamepadB; break;
      case kStandardPadX: mask = kDkc1GamepadX; break;
      case kStandardPadY: mask = kDkc1GamepadY; break;
      case kStandardPadBack: mask = kDkc1GamepadBack; break;
      case kStandardPadGuide: mask = kDkc1GamepadGuide; break;
      case kStandardPadStart: mask = kDkc1GamepadStart; break;
      case kStandardPadLeftStick: mask = kDkc1GamepadLeftStick; break;
      case kStandardPadRightStick: mask = kDkc1GamepadRightStick; break;
      case kStandardPadLeftShoulder:
        mask = kDkc1GamepadLeftShoulder;
        break;
      case kStandardPadRightShoulder:
        mask = kDkc1GamepadRightShoulder;
        break;
      case kStandardPadDpadUp: mask = kDkc1GamepadDpadUp; break;
      case kStandardPadDpadDown: mask = kDkc1GamepadDpadDown; break;
      case kStandardPadDpadLeft: mask = kDkc1GamepadDpadLeft; break;
      case kStandardPadDpadRight: mask = kDkc1GamepadDpadRight; break;
      default: break;
    }
    if (mask == kDkc1GamepadDpadUp && gamepad->left_y > deadzone) return true;
    if (mask == kDkc1GamepadDpadDown && gamepad->left_y < -deadzone) return true;
    if (mask == kDkc1GamepadDpadLeft && gamepad->left_x < -deadzone) return true;
    if (mask == kDkc1GamepadDpadRight && gamepad->left_x > deadzone) return true;
    return mask && (gamepad->buttons & mask) != 0;
  }
  if (!DKC1_PAD_IS_AXIS(binding)) return false;
  int axis = DKC1_PAD_AXIS_CODE(binding);
  bool positive = DKC1_PAD_AXIS_POSITIVE(binding) != 0;
  int value = 0;
...
uint32_t Dkc1MapGamepad(uint32_t buttons, int16_t left_x, int16_t left_y,
                        int16_t deadzone) {
  uint32_t input = 0;
  if (buttons & kDkc1GamepadA) input |= 1u << 0; /* SNES B */
  if (buttons & kDkc1GamepadX) input |= 1u << 1; /* SNES Y */
  if (buttons & kDkc1GamepadBack) input |= 1u << 2;
  if (buttons & kDkc1GamepadStart) input |= 1u << 3;
  if ((buttons & kDkc1GamepadDpadUp) || left_y > deadzone)
    input |= 1u << 4;
  if ((buttons & kDkc1GamepadDpadDown) || left_y < -deadzone)
    input |= 1u << 5;
  if ((buttons & kDkc1GamepadDpadLeft) || left_x < -deadzone)
    input |= 1u << 6;
  if ((buttons & kDkc1GamepadDpadRight) || left_x > deadzone)
    input |= 1u << 7;
  if (buttons & kDkc1GamepadB) input |= 1u << 8; /* SNES A */
  if (buttons & kDkc1GamepadY) input |= 1u << 9; /* SNES X */
  if (buttons & kDkc1GamepadLeftShoulder) input |= 1u << 10;
  if (buttons & kDkc1GamepadRightShoulder) input |= 1u << 11;
  return input;
}
...
uint32_t Dkc1MapHostActions(uint8_t left_trigger, uint8_t right_trigger,
                            uint8_t threshold) {
  uint32_t actions = 0;
  if (left_trigger > threshold) actions |= kDkc1HostRewind;
  if (right_trigger > threshold) actions |= kDkc1HostFastForward;
  return actions;
}
...
uint32_t Dkc1ApplyAssistGate(uint32_t mapped_actions,
  ...
  const uint32_t assist_mask =
      ...
      kDkc1HostSaveState | kDkc1HostLoadState;
...
uint32_t Dkc1RoutePlayerInputsWithBindings(
  ...
  for (int player = 0; player < kDkc1DesktopPlayerCount; player++) {
    uint32_t input = 0;
    if (player_sources[player] == kDkc1InputSourceKeyboard ||
        player_sources[player] == kDkc1InputSourceBoth) {
      input = keyboard_inputs[player];
    }
    if ((player_sources[player] == kDkc1InputSourceGamepad ||
         player_sources[player] == kDkc1InputSourceBoth) &&
               gamepads && next_gamepad < gamepad_count) {
      int percent = ClampInt(deadzone_percent[player], 0, 100);
      int deadzone = (32767 * percent + 50) / 100;
      input |= Dkc1MapGamepadBindings(
...
uint32_t Dkc1RoutePlayerInputs(
  ...
  for (int player = 0; player < kDkc1DesktopPlayerCount; player++) {
    ...
    if ((player_sources[player] == kDkc1InputSourceGamepad ||
         player_sources[player] == kDkc1InputSourceBoth) &&
               gamepads && next_gamepad < gamepad_count) {
      const Dkc1GamepadState *gamepad = &gamepads[next_gamepad++];
      int percent = ClampInt(deadzone_percent[player], 0, 100);
      int deadzone = (32767 * percent + 50) / 100;
      input |= Dkc1MapGamepad(gamepad->buttons, gamepad->left_x,
                             gamepad->left_y, (int16_t)deadzone);
      actions |= Dkc1MapHostActions(gamepad->left_trigger,
