...
Each real image presents for the whole divisor unless a midpoint of its own
follows it, in which case the pair splits the divisor (one refresh each on
120 Hz). The split is decided per frame from whether interpolation actually
produced a midpoint, never from the feature switch alone: a frame whose
interpolation was bypassed (pixel inspect, provenance overlay, single step,
layer isolation) or whose worker was cancelled keeps every refresh. A
half-divisor present with no partner would otherwise release the next
swap-chain slot after one refresh and emulation would advance at the display
rate. That was the v0.0.16 double-speed report: the first click in the game
window armed provenance tracking for the rest of the session, every later
frame skipped interpolation, and the host kept presenting real frames with
sync interval 1 on a 120 Hz display (the release build measured 8.33 ms per
emulated frame from the click onward). Click-to-inspect now arms provenance
only until the inspection resolves, so smoothing resumes two frames after a
click, and the DwmFlush fallback applies the same rule to its compositor
passes (`runner/win32_present_cadence.h`, `tests/test_win32_present_cadence.c`).
