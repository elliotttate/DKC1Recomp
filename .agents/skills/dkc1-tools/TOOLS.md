...
| `scripts/package_windows.py --build BUILD --out DIR --version X.Y.Z` | After a clean committed `build_windows.ps1`, creates the Windows SDL player ZIP (stock and Dixie executables, SDL2, licenses, docs), embedded `BUILDINFO.json` hashes and a SHA-256 sidecar from a fixed allowlist. Requires committed source and refuses to overwrite; never gathers private ROM/state/build directories. |
| `build_windows.ps1 -Rom ROM [-SkipDixie]` | Windows player build: generates the stock and Dixie sources (`scripts/generate_dixie_sources.py` applies the embedded IPS in Python), configures CMake with the Dixie sibling, builds `DKC1Recomp.exe`/`dkc1_dixie_desktop.exe`, and runs CTest plus the public suite. The SDL host presents through Direct3D 11 (`runner/windows_present.c`, HLSL from `scripts/generate_windows_hlsl.py`) with OpenGL as fallback; `DKC1_PRESENTER=opengl`, `DKC1_FRAMEGEN=1|force`, `DKC1_PRESENT_HZ`, `DKC1_PACING_LOG` and `DKC1_SQUARE_PIXELS` mirror the native host's switches. |
...
--version 0.0.17` packages both Windows executables, SDL2, licenses and user
...
states. Never use a tester's only slot. Since v0.0.17 the SDL host writes the
native host's `dkc1.pacing.v5` log (`"host":"sdl"`, presenter `d3d11` or
`opengl`, pacing `waitable`/`dwmflush`/`timer`, DXGI scanout statistics);
`tools/analyze_pacing.py` summarizes it. `DKC1Recomp.exe --graphics-test` now
runs the OpenGL and Direct3D synthetic tests in turn (a Direct3D failure adds
100 to the exit code). Tier-2 coverage journals go to `NUL` in the player host
unless `SNESRECOMP_TIER2_CAPTURE=1` or an explicit tier-2 path is set.
